...
run-asgi:
	uvicorn core.asgi:application --host 0.0.0.0 --port 1035 --reload
run:
	python manage.py runserver 0.0.0.0:1035
...
run-wsgi:
	gunicorn core.wsgi:application --bind 0.0.0.0:1035
tunnel:
	jprq http 1035 -s escape-api
open-bash:
...
startproject:
	django-admin startproject $(name) core .
model:
	cd apps/$(app) && mkdir -p models && touch models/__init__.py && mv models.py models/$(name).py 